...
def get_word_with_least_vowels(input):
    ...
    x=len(input)
    for i in range(x):
        lengths.append(len(set(input[i])&set("aeiou")))
    return input[lengths.index(min(lengths))]
...
def get_min_max_words(input):
    ...
    x=len(input);
    for i in range(x):
        inlen.append(len(input[i]))

    return input[inlen.index(min(inlen))],input[inlen.index(max(inlen))]# replace this calls to min and max
