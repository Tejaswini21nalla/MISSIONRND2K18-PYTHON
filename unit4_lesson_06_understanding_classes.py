...
def test_classes_init_initializer():
    ...
    class TestQueue:
        def __init__(self):
            assert 'Entered here !', "Entered here !"
...
def test_classes_bound_and_unbound_methods():
    ...
    assert AttributeError ,"function object has no attribute __self__"== TestQueue.push.__self__   #unbound method
...
class Person:
    ...
    def get_bmi_result(self):
        p=Person(self.name,self.age,self.height,self.weight)
        bmi = p.weight/((p.height*0.3048)**2);
        if bmi<18.5:
              return "underweight"
        elif bmi<25:
                return "healthy"
