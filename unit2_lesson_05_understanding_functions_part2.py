...
def test_function_call_with_keyword_arguments():
    ...
    #No above one is not true as positional argument is followed by keyword argument which is wrong.
    #thecorrect one is
    assert [10,20,3,4] ==demo(10,second=20)
...
def test_function_with_keyword_args():
    ...
    assert { } == demo_with_keyword_args(name="jack") # what do you observe here?returning empty dictionary
...
def simple_format(format, *args):
    ...
    """
    for x in format:
        if x.isdigit() and int(x)<len(args):
            i=list(format)
            i[i.index(x)-1]=''
            i[i.index(x)]=args[int(x)]
            format="".join(i)
    return format
...
-*args is used to explain that we can pass variable no.of arguments
-**kwargs is used to pass variable no.of named arguments
-when we return args we get arguments in tuple where as kwargs as dict
"""
